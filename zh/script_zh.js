...
(async () => {
  ...
  try {
    ...
    alert(
      "抱歉，您的浏览器不受支持！\n 为了获得最佳体验，请使用以下任意受支持的浏览器：\n Chrome、Firefox、Safari、QQ 或微信。"
    );
  ...
  function stop() {
    ...
    stopButton.innerHTML = '加工... <i class="fa fa-spinner fa-spin"></i>';
  ...
  function submit() {
    ...
    if (
      ...
      alert("请检查并提供您的实际电子邮件地址，以确保收到报告。");
      document.getElementById("email").scrollIntoView(true);
    } else if (!v_mobile) {
      alert("请提供真实手机号码！ （包括+国家代码）\n 示例：+XX-XXXXXXXXX");
